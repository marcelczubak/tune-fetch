...
const NewPlaylist = ({songs, onRemove, onSave, playlistName, setPlaylistName, playlistToView, setPlaylistToView, setPlaylists, playlists}) => {
  ...
  return (
    <div className='flex flex-col justify-between h-full pb-4 bg-stone-200/80 min-h-[20rem] rounded-lg mb-4 shadow-md'>
      <div className='pt-6 px-4'>
        {!isViewing && (
          <input
            className='mb-4 text-3xl text-black/60 font-semibold w-full bg-gray-900/5 rounded-md px-2 py-1'
        ...
        <div className='flex items-center justify-between'>
            <div className='px-2 relative mb-2'>
                <h4 className='uppercase text-sm font-bold text-gray-400'>Currently Viewing</h4>
                <h2 className='mb-4 text-3xl text-gray-900 font-semibold'>{playlistToView.name}</h2>
            </div>
            <div className='mr-1'>
        ...
        {(isViewing && songsToRender.length == 0) && <h3 className='text-gray-500/80 text-center'>Your playlist is empty</h3>}
